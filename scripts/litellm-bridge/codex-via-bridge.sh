#!/usr/bin/env bash
# Run OpenAI Codex CLI against the local LiteLLM bridge (must already be running).
# Usage:
#   ./scripts/litellm-bridge/codex-via-bridge.sh
#   ./scripts/litellm-bridge/codex-via-bridge.sh -- full-auto
#   ./scripts/litellm-bridge/codex-via-bridge.sh ~/my/repo -- model composer-2-fast
set -euo pipefail
